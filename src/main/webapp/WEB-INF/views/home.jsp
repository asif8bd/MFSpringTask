	...
	<div class="container">
		<c:if test="${!empty alldata}">
			...
			<table class="table table-striped">
				...
				<c:forEach var="row" items="${alldata}" varStatus="theCount">
					<tr>
						<td><c:out value="${row.studentId}" /></td>
						<td><c:out value="${row.studentName}" /></td>
						<td><c:out value="${row.studentEmail}" /></td>
						<td>
							<a href="edit.html?n=${theCount.index}&studentId=${row.studentId}&studentName=${row.studentName}&studentEmail=${row.studentEmail}" class="btn btn-success" />Edit</a>
						</td>
						<td>
							<a href="delete.html?n=${theCount.index}" class="btn btn-danger" />Delete</a>
						</td>
					</tr>
				</c:forEach>
			</table>
